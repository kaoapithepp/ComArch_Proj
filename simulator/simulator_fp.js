...
function convertToBinary(dec, BITCOUNT) {
    let binaryStr, bitCount = BITCOUNT;
    if (dec >= 0) {
      let twosComp = dec.toString(2);
      binaryStr = padAndChop(twosComp, '0', (bitCount || twosComp.length));
    } else {
        binaryStr = (Math.pow(2, bitCount) + dec).toString(2);
...
function nandOps(regA, regB, destReg) {
    ...
    // fold via reduce
    const str = regAext.reduce((prev, curr, index) => 
        `${1-(Number(regAext.charAt(index)) * Number(regBext.charAt(index)))}`,'');
...
try {
    ...
    while(noHalted){
        identifierBinary(convertToBinary(Number(MEM_DECIMAL_ARRAY[pc]), 25));
        displayState();
    }
